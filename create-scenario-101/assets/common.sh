...
for module in "${MODULES[@]}"; do
    ...
    # 4.4 interpolated during build by maven assembly plugin
    if [ -z "$CLASSPATH" ]; then
        CLASSPATH="$JET_HOME/opt/hazelcast-jet-${module}-4.4.jar"
    else
        CLASSPATH="$JET_HOME/opt/hazelcast-jet-${module}-4.4.jar:$CLASSPATH"
    fi
done
